...
export default function VendorLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    // Simulated authentication - accepts anyone
    // We just check if fields are roughly valid (handled by HTML required)
    setTimeout(() => {
        toast({ title: "Welcome", description: "Logged in to Admin Portal successfully." });
        router.push("/vendor-tools/dashboard/analytics");
        setIsLoading(false);
    }, 1000);
  };
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col lg:flex-row font-inter overflow-hidden">
      ...
      <div className="hidden lg:flex m-4 rounded-4xl lg:w-1/2 bg-gradient-to-tr from-slate-950 to-indigo-950 items-center justify-center relative border-r p-12 overflow-hidden bg-dot-white/[0.2]">
        <div className="absolute pointer-events-none inset-0 flex items-center justify-center [mask-image:radial-gradient(ellipse_at_center,transparent_20%,black)]"></div>
        <div className="relative z-10 w-full max-w-2xl h-full flex flex-col justify-end pb-12">
            <div className="h-[120%] w-full absolute -bottom-20 left-0">
               <World globeConfig={globeConfig} data={sampleArcs} />
            </div>
            <div className="relative z-20 text-center text-white space-y-4 max-w-lg mx-auto pointer-events-none select-none">
                <h2 className="text-4xl font-bold tracking-tight">Global Infrastructure</h2>
                <p className="text-blue-100 text-lg">Secure, distributed network for sovereign AI deployment across borders.</p>
            </div>
        </div>
      </div>
